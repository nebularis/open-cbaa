...
3. **Catalog.** An OASIS XML catalog, `ontology/catalog-v001.xml`, generated by
   `tools/lattice_catalog.py` (D16). It maps Open CBAA's own ontology IRIs to their files, each
   released LATTICE version IRI to its raw file at its release tag (LATTICE ADR-A86, second
   addendum), each LATTICE ontology's unversioned IRI to its latest release, and the external
   vocabularies LATTICE imports as LATTICE's own catalog maps them. Protégé and the OWL API
   read it from the opened ontology's directory, so an Open CBAA ontology opens with its
   LATTICE imports resolved and no checkout. Python tooling uses the resolver in LATTICE's
   `tools/ontology_catalog.py`, since `rdflib` has no catalog support. The submodule is still
   needed for LATTICE's compilers and checks.
