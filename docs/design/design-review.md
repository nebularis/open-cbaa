...
I would adopt some parts of LATTICE as Open CBAA’s imported substrate today, but retain others as a design reference and future interoperability target.
...
- Core dependencies have known logical defects, and Open CBAA requires further upstream changes before it can import several layers. This makes Open CBAA’s core design contingent on another pre-1.0 project.
- LATTICE gives generic mechanism, not the decisive domain semantics. Statements, wording bindings, authority envelopes, market vocabulary, materiality, and clause semantics remain Open CBAA work.
...
Replace D7’s “LATTICE is imported” position with: Open CBAA is *LATTICE-informed and LATTICE-compatible*, but normatively independent. Build Open CBAA’s small semantic core directly on OWL, SKOS, SHACL, and PROV-O. Keep optional alignment modules to LATTICE terms.

A LATTICE layer should only become a direct import after a layer-specific acceptance gate shows that it:

1. Covers a required Open CBAA capability without a local workaround.
...
weighting. LATTICE and Open CBAA have one maintainer, and Open CBAA is LATTICE's first
consumer, so upstream responds to Open CBAA's requirements directly. Both are developed by
agents under human governance of architectural decisions, so an Open CBAA feature needing an
...
- **Ratification lags delivery.** ADR-A86, on which the pinning design relies, is implemented
  ...
  At `ddeaecf` ratification has caught up: ADR-A86 and every other ADR Open CBAA relies on are
  Accepted, and the status records match the commits. Automation remains open (L11).
- **External adoption.** A market-facing specification that depends on a single-maintainer
  ...
  compilation.
- **Design pull.** Since upstream changes on request, Open CBAA's design may drift towards what
